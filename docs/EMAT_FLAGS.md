...
| `EMAT_RG_CACHE_RETENTION` | ON (AUTO; `=0` forces legacy FIFO, `=1` forces retention) | `flags::tri_state`, unset/unrecognized = AUTO = ON | [emat_arrow_reader.rs (`rg_cache_retention_resolved`)](../crates/ematix-flow-core/src/emat_arrow_reader.rs) | Σ.AI.6e — RG decode-cache eviction policy: segmented LRU with admission-on-second-touch (probation + protected ≤ 4/5 capacity; probation-first eviction). Fixes the Q09 SF=100 collapse: the legacy FIFO evicts the warmup-seeded working set once per-cycle insert traffic exceeds capacity (trials 1–2 fast at ~6.5 s, trial 3+ 16–50 s decode-bound); retention keeps twice-touched entries protected so one-touch floods die in probation. Unit benches: flood hot-set 0/16 → 16/16, Q09-shaped steady state 0/40 → 40/40, LRU-friendly loop exact parity. Resolved at cache construction. Probes: `retention_stats()` = (admits, rejects, protected_evictions). 32 GB-box full-suite A/B pending (docs/plans/MEMORY_BUDGET.md option 4). |
