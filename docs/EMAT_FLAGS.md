...
| `EMAT_MEM_FLOOR_BYTES` | ON (Linux; auto = max(1 GiB, 3% RAM); set =0 to disable) | bytes | [mem_pool.rs](../crates/ematix-flow-core/src/mem_pool.rs) (applied in [preset.rs](../crates/ematix-flow-core/src/preset.rs)) | Σ.AI.6c ElasticFloorPool OOM guard: sessions stay UNBOUNDED while the machine is healthy (zero perf tax — unlike the refuted `EMAT_MEM_POOL_FRACTION` blanket cap), but when system `MemAvailable` minus a requested allocation would sink below the floor, `try_grow` returns a recoverable per-query `ResourcesExhausted` instead of letting the kernel OOM-kill the process (the parted-SF100 Q07 failure mode). MemAvailable read from `/proc/meminfo`, ~25 ms TTL cache; non-Linux (no sensor) = unguarded as before. SCOPE (run6): guards DataFusion-TRACKED consumers (joins/sorts/aggs/repartition); untracked ematix decode-buffer pressure (e.g. the parted multi-file union scan) is not seen by `try_grow` — decode-pressure shedding is the planned fix (docs/plans/MEMORY_BUDGET.md). Precedence: caller `RuntimeEnv` > `EMAT_MEM_POOL_FRACTION` opt-in cap > this floor > unbounded. |
...
| `EMAT_RG_CACHE_RETENTION` | ON (AUTO; `=0` forces legacy FIFO, `=1` forces retention) | `flags::tri_state`, unset/unrecognized = AUTO = ON | [emat_arrow_reader.rs (`rg_cache_retention_resolved`)](../crates/ematix-flow-core/src/emat_arrow_reader.rs) | Σ.AI.6e — RG decode-cache eviction policy: segmented LRU with admission-on-second-touch (probation + protected ≤ 4/5 capacity; probation-first eviction). Fixes the Q09 SF=100 collapse: the legacy FIFO evicts the warmup-seeded working set once per-cycle insert traffic exceeds capacity (trials 1–2 fast at ~6.5 s, trial 3+ 16–50 s decode-bound); retention keeps twice-touched entries protected so one-touch floods die in probation. Unit benches: flood hot-set 0/16 → 16/16, Q09-shaped steady state 0/40 → 40/40, LRU-friendly loop exact parity. Resolved at cache construction. Σ.AI.6f — ghost-assisted adaptive demotion (ARC-lite) fixes the retention *freeze* found via field counters on AWS SF=100: once the cache filled, protected never demoted (`retention(admit+0 … prot_evict+0)` on every later query, tens of thousands of rejects) — whoever filled protected first owned it forever. Keys evicted from probation un-promoted go to a bounded keys-only ghost list; a re-request of a ghosted key demotes protected-LRU entries that are stale (untouched since that key's eviction) to probation-MRU. One-touch floods never ghost-hit (scan resistance preserved); live protected sets have fresh stamps and are never demoted. Probes: `retention_stats()` = (admits, rejects, protected_evictions), `retention_ghost_stats()` = (ghost_hits, ghost_demotions). 32 GB-box full-suite A/B pending (docs/plans/MEMORY_BUDGET.md option 4). |
...
| `EMAT_LOWCARD_GROUPBY_BOOST` | AUTO (ON iff effectively solo; `=1` force-on diagnostic, `=0` force-off) | `flags::tri_state` | [auto_target_partitions.rs](../crates/ematix-flow-core/src/auto_target_partitions.rs) | Gate-B: join-free low-card GROUP BY at ROW-GROUP granularity (`target_partitions = num_rgs`, capped; Q01 SF=10 −7% at P=58). AUTO requires `resolved_core_share() == available_cores()` — the boost vanishes under multi-process load (oversubscription at 10 streams = −38% QPH). |
| `EMAT_LOWCARD_BOOST_CAP` | 8 | usize | [auto_target_partitions.rs](../crates/ematix-flow-core/src/auto_target_partitions.rs) | Gate-B partition ceiling multiplier: cap = N × `resolved_core_share()` (M4 Max solo: 8×14 = 112). `=0` effectively disables the boost. |
| `EMAT_MEM_POOL_FRACTION` | **off** (unbounded) | fraction of RAM, `(0,1]` | [mem_pool.rs](../crates/ematix-flow-core/src/mem_pool.rs) (applied in [preset.rs](../crates/ematix-flow-core/src/preset.rs)) | Σ.AI.6: OPT-IN bounded DataFusion memory pool at `f × physical RAM`. Isolated Q09 SF100 A/B won hugely (unbounded 94.3 s → 6.58 s @0.7); the full-suite zero-override re-bench REFUTED it as a default (flat SF100 82.5→140.2 s — Q10 spills to EBS; parted SF100 livelocks — DF53 hash-join builds cannot spill and deadlock under a Greedy cap). Use for memory-tight deployments preferring per-query `ResourcesExhausted`/backpressure over a kernel OOM-kill; recommended value 0.7. Caller-installed `RuntimeEnv` wins; unknown-RAM platforms stay unbounded. |
| `EMAT_MESH` | AUTO (`=1` always distribute, `=0` never; unset = per-query decision) | `flags::tri_state` | [mesh_gate.rs](../crates/ematix-flow-distributed/src/mesh_gate.rs) | Adaptive mesh gate on the datafusion-distributed stage splitter (peers-configured sessions only: `DistributedBackend::build_context` + the campaign harness). **Recognized values: only `1`/`true` (force distribute) and `0`/`false` (force single-node); anything else — including `on`, `off`, `auto`, `yes` — resolves to AUTO.** `=0` keeps plans byte-identical to single-node with peers configured but unused. AUTO sums `total_byte_size` across the plan's scan leaves per query at plan time: known sum ≥ `EMAT_MESH_MIN_BYTES` ⇒ distribute, below ⇒ single-node (skips mesh coordination overhead — distributed Q22 ~120 ms vs ~24 ms single-node at SF=10); ALL leaves stat-less ⇒ distribute (preserves pre-gate behavior). **Behavior change (Unreleased): peer-configured sessions previously always distributed; the default is now AUTO. Pin `EMAT_MESH=1` to reproduce pre-gate benchmark numbers.** The resolved gate (mode + threshold) is logged at session build; campaign JSON reports the per-query outcome as `QueryStats::plan_mode`. |
...
| `EMAT_MESH_FLOAT_EQ_VETO` | ON (`=0` disables — debugging only, wrong answers possible) | `flags::tri_state`, unset = ON | [mesh_gate.rs](../crates/ematix-flow-distributed/src/mesh_gate.rs) | Σ.Q15.FP correctness veto: force single-node when the plan equality-compares two computed FLOAT expressions (hash-join key, nested-loop join filter, or filter predicate) with an order-sensitive float reduction (`sum`/`avg` over floats) below. Distributed partial-merge order is nondeterministic and f64 addition is not associative, so the two evaluations can differ by one ULP and the equality silently drops rows — TPC-H Q15 returned 0 rows on ~1/3 of mesh runs at SF=100 (2026-07-12). `min`/`max` are order-exact and do not trip it (Q02 keeps its mesh plan); literal comparisons are exempt. **Overrides `EMAT_MESH=1`** — a forced-mesh wrong answer is still a wrong answer. Durable fix (deterministic distributed float reduction) is a post-1.0 arc. |
| `EMAT_MESH_MIN_BYTES` | `8_388_608` (8 MiB) | u64 bytes (**bare integer only**); **calibrated 2026-07 from the SF1/SF10 crossover campaign** | [mesh_gate.rs](../crates/ematix-flow-distributed/src/mesh_gate.rs) | `EMAT_MESH` AUTO threshold: distribute a query when its scan leaves' summed known `total_byte_size` is ≥ this. On 8-way-partitioned data the mesh wins/ties across the whole measured 1 MiB–3.8 GiB range, so the default sits low (8 MiB) to distribute broadly while skipping trivial scans. Compared against `Statistics::total_byte_size`, which for parquet is the row groups' **uncompressed** size (≈2–4× on-disk file size, minus projection/pushdown), so this is NOT "≈ file size". Only a bare integer parses (e.g. `8388608`); suffixed/scientific forms (`8MiB`, `1e7`) are rejected and **warn at session build** before falling back to the default. |
...
| `EMAT_COND_CACHE_BYTES` | `268435456` (256 MiB) | usize bytes; `0` disables | [cond_cache.rs](../crates/ematix-flow-core/src/cond_cache.rs) | Σ.CC query condition cache: `BridgeFilter::build_bitmap` results (per file+mtime+len, row group, static-predicate fingerprint) served from a bounded LRU, so REPEATED predicates skip every predicate-column decode for the row group. Runtime join blooms/sets never cache (per-query); file rewrites change the identity key and decay out. Methodology: accelerates repeat queries — trials 2..N and `med(3-5)` benefit, `first_trial_ms` stays cold-honest. |
| `EMAT_SIDECAR_SQL` | on | tri-state (`tri_state`) | [sidecar_exec.rs](../crates/ematix-flow-core/src/sidecar_exec.rs) | Σ.SC P3W SQL path onto sidecar indexes: when a part has an on-disk sidecar and the query pushes a covered `col = <int>` point predicate, the fast-parquet provider swaps that part's scan for a `SidecarLookupExec` (or an empty relation when footer bounds prove the key absent — the parted range-prune). `0` disables; inert without a sidecar file, and stands down under `EMAT_EXACT_PUSHDOWN=1` (the exec answers only the eq, so it must not ride exactness claims). |
