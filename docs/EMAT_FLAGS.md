...
| `EMAT_MEM_POOL_FRACTION` | ON, `0.7` (set =0 to disable) | fraction of RAM, `(0,1]` | [mem_pool.rs](../crates/ematix-flow-core/src/mem_pool.rs) (applied in [preset.rs](../crates/ematix-flow-core/src/preset.rs)) | Σ.AI.6: bounded-by-default DataFusion memory pool at `0.7 × physical RAM`. THE Q09 SF100 memory-cliff fix (unbounded 94.3 s → 6.58 s on the 32 GB campaign box; DuckDB parity) and the parted-SF100 OOM-kill guard — memory-aware operators get backpressure instead of ballooning and evicting the page cache; over-cap queries fail with a recoverable `ResourcesExhausted` instead of a kernel OOM-kill. Caller-installed `RuntimeEnv` always wins; unknown-RAM platforms stay unbounded. `=0`/`false`/`off` restores legacy unbounded; a float in `(0,1]` overrides the fraction. |
