...
| `EMAT_MESH_MIN_BYTES` | `4_294_967_296` (4 GiB) | u64 bytes (**bare integer only**); **uncalibrated placeholder pending campaign calibration** | [mesh_gate.rs](../crates/ematix-flow-distributed/src/mesh_gate.rs) | `EMAT_MESH` AUTO threshold: distribute a query when its scan leaves' summed known `total_byte_size` is ≥ this. Compared against `Statistics::total_byte_size`, which for parquet is the row groups' **uncompressed** size (≈2–4× on-disk file size, minus projection/pushdown), so this is NOT "≈ file size". Only a bare integer parses (e.g. `4294967296`); suffixed/scientific forms (`4GiB`, `1e9`) are rejected and **warn at session build** before falling back to the default. |
