...
| `EMAT_MESH_MIN_BYTES` | `4_294_967_296` (4 GiB) | u64 bytes; **initial value pending campaign calibration** | [mesh_gate.rs](../crates/ematix-flow-distributed/src/mesh_gate.rs) | `EMAT_MESH` AUTO threshold: distribute a query when its scan leaves' summed known `total_byte_size` is ≥ this. Unparseable values fall back to the default. |
