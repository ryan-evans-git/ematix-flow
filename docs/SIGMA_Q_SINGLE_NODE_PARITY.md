...
| L9 | **Adaptive query execution**: runtime sideband from HashJoinExec → probe scan | 🔵 PROPOSED — high build cost, high ceiling | Capture build-side blooms / build-side cardinality as a side-effect of `HashJoinExec`'s existing build phase. Plumb them back into the probe-side `EmatixFastParquetExec` BridgeFilter at execute time, after the build is done but before probe-batch processing starts. Sub-tasks: **(a)** sideband channel on the exec (`Arc<RwLock<Option<Vec<ColumnPredicate>>>>`); **(b)** hook in HashJoinExec build phase to emit i64 bloom from the build side's join key column; **(c)** probe-scan watcher that picks up the sideband and merges into the BridgeFilter before masked-decode begins. Unlocks Σ.Q.L4′'s residual Q07 NEG (the "capture-as-side-effect" path the slice 4 NEG diagnosis pointed at) AND is the foundation layer for runtime AQE — same channel later carries adaptive-skew detection, late-arrival selectivity, dynamic partition rebalancing. Estimated 3-5 days build + bench. Sister to [[sigma-l-adaptive-runtime]]'s shipped primitives (which are per-query / inter-query / per-batch); L9 reaches further into **mid-query** plan adaptation. |
