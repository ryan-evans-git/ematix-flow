...
## Position within the broader Σ.T roadmap

This plan is the implementation of **L1 (sidecar Phase 1 read-side)** and
**L15 (sidecar Phase 2 + storage layer write-side)** from
[`docs/PHASE_SIGMA_T_SF10_WEAKNESS_CLOSURE_V5.md`](../PHASE_SIGMA_T_SF10_WEAKNESS_CLOSURE_V5.md).
V5 classifies both as **Tier 5 — scale-specific** (universal benefit only
at SF=10+; SF=1 sees ~0 pp). V5's sequence runs this work as a parallel
track alongside:

- **Tier 1 headline path** (L13 custom hash join → L14 dict-preserved →
  L8 CBO → L17 learned join order → L10 dynamic filter propagation) —
  universal levers whose magnitude *amplifies* with scale.
- **Tier 2 depth path** (L4 → L7 → L12 → L11) — universal flat-tax
  levers.

If we have a single engineer, V5's prescription is to ship **L3 PGO
release build** (1 week, ~3-5pp universal lift) *before* resuming this
sidecar work. L3 has zero blast radius and absorbs the codegen tax for
every subsequent lever.

The cross-references run both ways: V5 §1 Tier 5 cites this doc for the
sidecar implementation plan; this doc treats V5 as the broader roadmap
that contextualises the sidecar work's expected scale-curve impact.
