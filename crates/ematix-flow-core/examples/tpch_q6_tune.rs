...
//! | Path                                       | Median (ms) |
//! |--------------------------------------------|-------------|
//! | DataFusion MemTable (default)              | 5.96        |
//! | Polars MemTable                            | 1.9         |
//! | Hand-written fused, single-thread          | 7-10        |
//! | **Hand-written fused, 14-thread**          | **1.0**     |
//!
//! That sets the floor for a future `FusedFilterSumExec` physical
//! operator. The architectural premise (mask materialization is the
//! gap; a fused kernel closes it) is confirmed. Issue #44 tracks the
//! integration work — wrapping the hand-written loop as a DataFusion
//! `ExecutionPlan` and adding the optimizer rule to recognize
//! `Aggregate(SUM) over Filter(predicate)`-shaped plans.
//!
//! Usage:
