...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut cache_prev: (u64, u64, u64, u64, u64) = (0, 0, 0, 0, 0);
    for n in query_subset {
        ...
        // admits/rejects/protected-evictions). Answers "where did this query's
        // cache hits go" without a profiler — cumulative counters are read
        // before/after each query via the public probes.
        if std::env::var_os("EMAT_CACHE_STATS").is_some() {
            if let Some(c) = ematix_flow_core::emat_arrow_reader::process_rg_decode_cache() {
                ...
                let (ph, pm, pad, prj, ppe) = cache_prev;
                eprintln!(
                    "  cache: hits+{} misses+{} entries={} | retention(admit+{} reject+{} prot_evict+{}) enabled={}",
                    h - ph, m - pm, entries, ad - pad, rj - prj, pe - ppe,
                    c.retention_enabled()
                );
                cache_prev = (h, m, ad, rj, pe);
            }
        }
